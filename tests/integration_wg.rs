...
fn spawn_daemon_with_peer(persistent_keepalive: Option<u16>) -> DaemonSetup {
    ...
    DaemonSetup {
        ...
        peer_port,
        keys,
        daemon_addr: format!("127.0.0.1:{daemon_port}")
            .parse()
            .expect("daemon addr"),
    }
}
...
fn recv_until(socket: &UdpSocket, deadline: Instant) -> Option<(Vec<u8>, SocketAddr)> {
...
fn test_handshake_complete() {
    ...
    assert_eq!(
        init_len, WG_HANDSHAKE_INIT_SZ,
        "init must be exactly 148 bytes"
    );
    socket
    ...
    let (reply, _src) = recv_until(&socket, deadline)
        .expect("daemon never produced a handshake response within 3 s of receiving the init");
    assert_eq!(
        wg_msg_type(&reply),
    ...
    let result = tunn.decapsulate(Some(setup.daemon_addr.ip()), &reply, &mut decap_buf);
...
fn drain_until_ipv4(socket: &UdpSocket, tunn: &mut Tunn, deadline: Instant) -> Option<Vec<u8>> {
    let mut buf = vec![0u8; 2048];
    while Instant::now() < deadline {
...
fn test_allowed_ips_violation_dropped() {
    ...
    let (reply, _) =
        recv_until(&socket, Instant::now() + Duration::from_secs(3)).expect("no response");
    let _ = tunn.decapsulate(Some(setup.daemon_addr.ip()), &reply, &mut buf);
...
fn test_handshake_flood_rate_limited() {
    ...
    assert!(
        sent >= 100,
        "must send at least 100 inits, only sent {sent}"
    );
...
fn test_endpoint_roaming() {
    ...
    socket_a
        ...
        .unwrap();
    let (reply, _) =
        recv_until(&socket_a, Instant::now() + Duration::from_secs(3)).expect("no response on A");
    let _ = tunn.decapsulate(Some(setup.daemon_addr.ip()), &reply, &mut buf);
