    ...
    T::Bitmap: BitmapReplace + NewBitmap + Clone,
{
    ...
    fn set_vring_enable(&mut self, index: u32, enable: bool) -> VhostUserResult<()> {
        // QEMU 8.2.x's vhost-user-net sends SET_VRING_ENABLE during its
        // probe phase before SET_FEATURES. The upstream check_feature call
        // here would reject that and break real QEMU compatibility.
        // Patched out to match libvhost-user / DPDK lenient behaviour.
        let vring = self
            .vrings
            .get(index as usize)
            .ok_or(VhostUserError::InvalidParam)?;
